...
public class FoodDbHelper extends SQLiteOpenHelper {
    ...
    // Se sube la versión a 4 para forzar la actualización a la nueva lógica de emails.
    ...
    // Se añade un email por defecto al admin para que pueda iniciar sesión.
    private void createDefaultAdmin(SQLiteDatabase db) {
        ContentValues adminValues = new ContentValues();
        adminValues.put(UserEntry.COLUMN_NAME_USERNAME, "admin");
        adminValues.put(UserEntry.COLUMN_NAME_EMAIL, "admin@example.com"); // Email por defecto
        ...
        Log.d(TAG, "Usuario administrador por defecto creado con email y contraseña segura.");
    }

    // HASHEAR LA CONTRASEÑA AL AÑADIR UN USUARIO ---
    public boolean addUser(String username, String password, String email) {
        SQLiteDatabase db = this.getWritableDatabase();
        ContentValues values = new ContentValues();
        values.put(UserEntry.COLUMN_NAME_USERNAME, username);
        values.put(UserEntry.COLUMN_NAME_EMAIL, email); // Se guarda el email
        ...
        values.put(UserEntry.COLUMN_NAME_IS_ADMIN, 0); //Nuevos usuarios no son admin

        long newRowId = db.insert(UserEntry.TABLE_NAME, null, values);
        return newRowId != -1;
    }

    // REHACER LA LÓGICA DE COMPROBACIÓN DEL USUARIO ---
    // Este método ya no se usa para el login con Firebase, pero se mantiene.
    ...
    // HASHEAR LA CONTRASEÑA AL ACTUALIZAR UN USUARIO ---
    ...
    // isAdmin ahora busca por EMAIL, nuestro nuevo identificador único.
    public boolean isAdmin(String email) {
        SQLiteDatabase db = this.getReadableDatabase();
        Cursor cursor = null;
        try {
            String query = "SELECT " + UserEntry.COLUMN_NAME_IS_ADMIN + " FROM " +
                    UserEntry.TABLE_NAME + " WHERE " + UserEntry.COLUMN_NAME_EMAIL + " = ?";
